...
const GITHUB_URL = "https://github.com/xinluli217/job_application_tracker";

const TAGS = ["Chrome Extension", "AI", "Notion API", "Automation"];

const STATS = [
  { value: "1-click", label: "Application logging",  sub: "From any job board" },
  { value: "~5s",     label: "Time to log",           sub: "vs. ~2 min manually" },
  { value: "2 APIs",  label: "Claude + Notion",       sub: "AI extraction & storage" },
  { value: "MV3",     label: "Manifest V3",           sub: "Chrome-standards compliant" },
];

const TECH = [
  { group: "Extension",  items: ["Chrome Manifest V3", "Content Script", "Popup UI"] },
  { group: "AI",         items: ["Anthropic API", "Claude Haiku", "Structured JSON output"] },
  { group: "Storage",    items: ["Notion API", "Chrome Storage Sync"] },
    ...
    <InnerLayout wide>
      <PageTransition>
        ...
        <Section label={t("detail.problem")}>
          {t("project.plugin-system.problem")}
        </Section>
        <Section label={t("detail.method")}>
          {t("project.plugin-system.method")}
        </Section>
        <Section label={t("detail.insight")}>
          {t("project.plugin-system.finding")}
        </Section>
        <Section label={t("detail.impact")}>
          {t("project.plugin-system.impact")}
        </Section>
